...
public class TorecWebEngineListener implements ChangeListener<State> {
	...
	private static Logger log = Logger.getLogger(TorecWebEngineListener.class);
	...
	private Serie serie;
	private boolean forceOpenWeb;
	private String startLink;

	public TorecWebEngineListener(WebView webview, TorecStage torecStage, Serie serie, boolean forceOpenWeb, String startLink) {
		this.webview = webview;
		this.webEngine = webview.getEngine();
		this.torecStage = torecStage;
		this.serie = serie;
		this.forceOpenWeb = forceOpenWeb;
	...
	public void changed(ObservableValue ov, State oldState, State newState) {	
		log.debug("Serie = " + serie + ", newState = " + newState);
		if (newState == State.SUCCEEDED) {				
			...
			if (torecStage == TorecStage.SEARCH_PAGE){
				log.debug("torecStage == TorecStage.SEARCH_PAGE");
				...
				if (torecStage == TorecStage.SEARCH_RESULTS_PAGE){
					log.debug("torecStage == TorecStage.SEARCH_RESULTS_PAGE");						
					handleSearchResultPage(stringOut);
				} else if (torecStage == TorecStage.SERIES_PAGE){
					log.debug("torecStage == TorecStage.SERIES_PAGE");						
					handleSeriesPage(stringOut);
				}
			}

		} else if (newState == State.FAILED){
			log.error("torecStage == TorecStage.FAILED");
			handleDone();
		}
	}

	private void handleDone() {
		log.info("torecStage == TorecStage.DONE");
		Platform.runLater(new Runnable() {
			@Override
			public void run() {
				Main.mainStage.getScene().setCursor(Cursor.DEFAULT);
				Main.setStatusTextOverride(null);
				HttpsClient.setWebEngineInProgress(false);
				HttpsClient.startNextWebEngineLoad();
			}
		});
	}
	...
	private void handleSeriesPage(StringWriter stringOut) {
		...
		if (serie.getMissingSubsEpisodesList().size() > 5){
